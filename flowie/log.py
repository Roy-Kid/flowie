...
log_configs = Configs(
    {
        ...
        "format": f"%(asctime)s %(name)s (%(filename)s:%(lineno)d) %(levelname)s - %(message)s",
    }
)
...
class _TimedRotatingFileHandlerSafe(logging.handlers.TimedRotatingFileHandler):
    def __init__(
        ...
        **kwargs,
        ...
        super(_TimedRotatingFileHandlerSafe, self).__init__(
            filename, when=when, backupCount=backup_count, **kwargs
        )
    ...
    def is_same_file(file1, file2):
        ...
        return (
            os.fstat(file1.fileno()).st_ino == os.fstat(file2.fileno()).st_ino
        )
...
def get_logger(
    ...
    if isFile:
        ...
        fh = _TimedRotatingFileHandlerSafe(
            filename=filename,
            when=when,
            interval=interval,
        ...
        if isFileColorized:
            fh.setFormatter(
                CustomFormatter(
                    auto_colorized=False, custom_format=custom_format
                )
            )
        else:
    ...
    if isStream:
        ...
        ch.setLevel(level)
        ch.setFormatter(CustomFormatter(custom_format, isColorized))
